...
class App
{
    ...
    public function addAttributes($name, $attributeTypeId, $attributeLabel, $options)
    {
        try
        {
            ...
            $name=str_replace(array(" ","-") ,"_",trim($name));
            $sql = "ALTER TABLE `sku` ADD `$name` varchar(45) NOT NULL AFTER `Price` ;";
            $stmt = $app->db->prepare($sql);
    ...
    public function updateSku($id, $postArray)
    {
        try
        {
            $temp = array();
            ...
            $sql = "update sku set " . $tempString . " where ID=$id";
            $app = App::getDBInstance();
            $stmt = $app->db->prepare($sql);
            $stmt->execute();
        } catch (PDOException $e)
    ...
    public function getTopSearchSkus($displayChart, $sort = 'value', $order = 'desc')
    {
        try
        {

            $sql = "SELECT sk.sku as label, skus.num_hits as value FROM sku_search_history as skus LEFT JOIN sku as sk ON skus.sku_id = sk.ID ORDER BY $sort $order";
            $app = App::getDBInstance();
            $stmt = $app->db->prepare($sql);
            $stmt->execute();
            $rows = $stmt->fetchALL(PDO::FETCH_ASSOC);
    ...
    public function checkSKU($sku)
    {
        try
        {
            $sql = "SELECT SKU FROM sku where SKU='$sku'";
            $app = App::getDBInstance();
            $stmt = $app->db->prepare($sql);
            $stmt->execute();
            $rows = $stmt->fetchAll(PDO::FETCH_OBJ);
